...
tic
[t{n},x4{n}] = integration(M,K,zeros(2),a,[1e-4, -1e-4, 0, 0]',100*1/27.5664,0,0,0,tol(n),tol(n));
time(n) = toc;
x1{n} = x4{n}(:,dofComp);
...
for i = n-1:-1:1;
    tic 
     [t{i},x4{i}] = integration(M,K,zeros(2),a,[1e-4, -1e-4, 0, 0]',100*1/27.5664,0,0,0,tol(i),tol(i));
     time(i) = toc;
     x1{i} = x4{i}(:,dofComp);
